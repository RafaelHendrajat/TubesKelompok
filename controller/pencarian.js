...
router.post("/hitungJumlah", async (req, res) => {
  ...
  const queryString =
    `SELECT count(target) as jumlah FROM book1 WHERE source LIKE ? AND book = '?'`;
...
router.post("/cariNama", async (req, res) => {
  const queryString =
    "SELECT id, source, target, weight FROM book1 WHERE source LIKE ? AND book = '?' ORDER BY source LIMIT ?,?";
    const name = "%"+req.body.name+"%";
  const book = req.body.book;
